...
// --- Benford's Law Theory Distribution ---
const BENFORD_THEORY = [30.1, 17.6, 12.5, 9.7, 7.9, 6.7, 5.8, 5.1, 4.6];
...
const AnalyticsView: React.FC<AnalyticsViewProps> = ({ allStores, dataType }) => {
    ...
    const riskAnalysisData = useMemo(() => {
        ...
        const budgetCounts = Array(9).fill(0);
        let actualTotal = 0;
        let budgetTotal = 0;
        ...
        stores.forEach(s => {
            ...
            s.raw.forEach(v => {
                ...
                if (firstDigit >= 1 && firstDigit <= 9) {
                    actualCounts[firstDigit - 1]++;
                    actualTotal++;
                }
            });
            // Budgets
            if (s.budget) {
                Object.values(s.budget).forEach(v => {
                    const firstDigit = parseInt(Math.abs(v).toString().charAt(0));
                    if (firstDigit >= 1 && firstDigit <= 9) {
                        budgetCounts[firstDigit - 1]++;
                        budgetTotal++;
                    }
                });
            }
        ...
        const benfordData = BENFORD_THEORY.map((theory, i) => ({
            digit: (i + 1).toString(),
            theory: theory,
            actual: actualTotal > 0 ? (actualCounts[i] / actualTotal) * 100 : 0,
            budget: budgetTotal > 0 ? (budgetCounts[i] / budgetTotal) * 100 : 0
        }));

        // MAD (Mean Absolute Deviation) Calculation
        const madActual = benfordData.reduce((acc, d) => acc + Math.abs(d.actual - d.theory), 0) / 9;
        const madBudget = benfordData.reduce((acc, d) => acc + Math.abs(d.budget - d.theory), 0) / 9;

        const benfordScore = { actual: madActual, budget: madBudget };

        return { riskReturn, cvDist, stabilityAge, riskyStores, giniHistory, benfordData, benfordScore };
    }, [stores, kpis]);
    ...
    const simAnalysisData = useMemo(() => {
        ...
        const estimatedCustomers = isSales ? Math.round((kpis.totalVolume as number) / 12 / basePrice) : Math.round((kpis.totalVolume as number) / 12);
    ...
    const geoAnalysisData = useMemo(() => {
        ...
        stores.forEach((s: StoreData) => s.dates.forEach((d: string) => monthSet.add(d.replace(/\//g, '-'))));
        const allMonths: string[] = Array.from(monthSet).sort((a: string, b: string) => new Date(a).getTime() - new Date(b).getTime());
        
        // Limit to last 360 months (30 years) for reasonable performance
        const targetMonths: string[] = allMonths.slice(-365);

        // Pre-calculate aggregate data for each frame (month)
        const frames = targetMonths.map((month: string) => {
            ...
            stores.forEach((s: StoreData) => {
                ...
                if (idx !== -1) {
                    ...
                    const idxPM = s.dates.findIndex((d: string) => d.replace(/\//g, '-') === prevMStr);
                    if (idxPM !== -1 && s.raw[idxPM] > 0) {
                        frameData[pref].mom += (val - s.raw[idxPM]) / s.raw[idxPM];
                    }

                    // YoY
                    const idxPY = s.dates.findIndex((d: string) => d.replace(/\//g, '-') === prevYStr);
        ...
        return { 
            frames, 
            kpis: {
                fastestGrowing: { name: fastestGrowing, val: (currentFrameData[fastestGrowing]?.yoy || 0) * 100 },
                declining: { name: declining, val: (currentFrameData[declining]?.yoy || 0) * 100 },
            ...
            case 'benford':
                return (
                    <div className="h-full flex flex-col">
                        <div className="flex justify-end gap-4 mb-2 text-[10px] font-bold">
                            <span className={`px-2 py-1 rounded ${riskAnalysisData.benfordScore.actual > 2.5 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                                実績データ信頼度: {riskAnalysisData.benfordScore.actual > 2.5 ? '要確認 (偏差大)' : '正常'} (MAD: {riskAnalysisData.benfordScore.actual.toFixed(2)})
                            </span>
                            <span className={`px-2 py-1 rounded ${riskAnalysisData.benfordScore.budget > 2.5 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                                予算データ信頼度: {riskAnalysisData.benfordScore.budget > 2.5 ? '作為的可能性あり' : '正常'} (MAD: {riskAnalysisData.benfordScore.budget.toFixed(2)})
                            </span>
                        </div>
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={riskAnalysisData.benfordData} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                <XAxis dataKey="digit" tick={{fontSize:10}} label={{ value: '先頭の数字 (1-9)', position: 'bottom', offset: 0, fontSize: 10 }} />
                                <YAxis unit="%" tick={{fontSize:10}} />
                                <Tooltip formatter={(v:number)=>v.toFixed(1)+'%'} contentStyle={{borderRadius:'12px', border:'none', boxShadow:'0 10px 15px -3px rgba(0,0,0,0.1)'}} />
                                <Legend wrapperStyle={{fontSize:'10px'}} />
                                <Area type="monotone" dataKey="theory" fill="#F59E0B" fillOpacity={0.1} stroke="#F59E0B" strokeWidth={2} strokeDasharray="5 5" name="理論値 (Benford)" />
                                <Bar dataKey="actual" fill="#005EB8" barSize={20} name="実績データ分布" radius={[4,4,0,0]} />
                                <Bar dataKey="budget" fill="#10B981" barSize={20} name="予算データ分布" radius={[4,4,0,0]} />
                            </ComposedChart>
                        </ResponsiveContainer>
                    </div>
                );
             ...
             <div className="w-full px-4 md:px-8 space-y-8 pb-32">
                ...
                {activeTab === 'risk' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
                        ...
                        <ChartCard id="benford" title="データ自然性監査 (Benford's Law Check)" className="lg:col-span-3 h-[400px]" onExpand={setExpandedChartId} info="数値改ざん検知" helpTitle="ベンフォードの法則" helpContent="自然に発生する数値データの先頭の桁は、特定の確率分布（1が約30%、9が約4.6%）に従います。これと大きく乖離する場合、人為的なデータ操作や異常なバイアスが含まれている可能性があります。">
                            {renderChartContent('benford')}
                        </ChartCard>
                    </div>
                )}
