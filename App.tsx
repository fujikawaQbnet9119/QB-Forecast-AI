...
type ViewType = 'data' | 'dashboard' | 'spot' | 'regional_spot' | 'regional_strategy' | 'analytics' | 'bench' | 'store' | 'simulation' | 'table' | 'logic' | 'guide' | 'validate' | 'validate_budget' | 'logic_flow' | 'marketing_design' | 'coach' | 'budget' | 'budget_comparison' | 'version_history';
...
const App: React.FC = () => {
    ...
    const isRestricted = (view: string) => {
        ...
        const executiveOnly = ['spot', 'regional_strategy', 'table', 'analytics', 'validate', 'validate_budget', 'logic', 'guide', 'budget', 'budget_comparison'];
        return executiveOnly.includes(view);
    };

    return (
        <div className="relative h-screen w-full bg-[#F8FAFC] text-[#1A1A1A] overflow-hidden">
            ...
            >
                <div className={`absolute top-4 left-4 z-40 transition-opacity duration-300 ${isSidebarOpen || isSidebarPinned ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                    <button 
                        ...
                        className="bg-white/80 backdrop-blur-sm p-2 rounded-lg shadow-md border border-gray-200 text-[#005EB8] hover:bg-white transition-transform hover:scale-105"
                        title="Show Menu"
                    >
                ...
                {isRestricted(currentView) && (
                    <div className="flex-1 flex flex-col items-center justify-center p-10 bg-white h-full">
                        ...
                        <h2 className="text-2xl font-black text-gray-800 mb-2">アクセス制限エリア</h2>
                        <p className="text-gray-500 font-bold mb-6">この機能を表示するには経営者モードでのログインが必要です。</p>
                        <button onClick={() => setCurrentView('dashboard')} className="bg-[#005EB8] text-white px-8 py-2 rounded-lg font-black text-xs uppercase tracking-widest">ダッシュボードへ戻る</button>
                    </div>
                )}
